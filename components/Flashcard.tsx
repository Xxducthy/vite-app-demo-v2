...
export const Flashcard: React.FC<FlashcardProps> = ({ 
  ...
  const StatusBadge = () => (
    <div className="absolute top-6 left-6 flex items-center gap-3 z-20">
        {sessionAttempts > 0 && (
            <div className="flex items-center gap-1.5 px-2 py-1 bg-slate-50/80 dark:bg-slate-800/80 backdrop-blur border border-slate-100 dark:border-slate-700 rounded-lg text-slate-400 dark:text-slate-500">
        ...
        {learningStreak !== undefined && (
            <div className="flex items-center gap-1 px-2 py-1 bg-slate-50/80 dark:bg-slate-800/80 backdrop-blur border border-slate-100 dark:border-slate-700 rounded-lg">
                <Trophy size={10} className={learningStreak > 0 ? "text-amber-500" : "text-slate-300 dark:text-slate-600"} />
                <div className="flex gap-0.5 ml-1">
                    {[...Array(3)].map((_, i) => (
                        <div key={i} className={`w-1.5 h-1.5 rounded-full transition-colors duration-300 ${i < learningStreak ? 'bg-amber-400' : 'bg-slate-200 dark:bg-slate-700'}`}></div>
                    ))}
                </div>
            </div>
        )}
      ...
      return (
          <div className="w-full max-w-lg bg-white dark:bg-slate-900 rounded-3xl shadow-xl dark:shadow-none border border-slate-100 dark:border-slate-800 flex flex-col overflow-hidden relative min-h-[500px]">
              ...
              <div className="absolute top-6 right-6 z-20">
                  <span className="text-[10px] font-bold uppercase text-slate-400 bg-slate-50 dark:bg-slate-800 px-2.5 py-1 rounded-full border border-slate-100 dark:border-slate-700">Spelling</span>
              </div>
              ...
              <div className="p-8 pt-16 flex-grow flex flex-col gap-6">
                  ...
                  <div className="flex justify-center">
                     <button onClick={() => speakText(word.term)} className="w-16 h-16 bg-indigo-50 dark:bg-indigo-900/30 rounded-full flex items-center justify-center text-indigo-600 dark:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 hover:scale-105 transition-all shadow-sm">
                         <Volume2 size={32} />
                     </button>
                  </div>
                      ...
                      {maskedMeanings.slice(0, 2).map((m, idx) => (
                          <div key={idx} className="bg-slate-50 dark:bg-slate-800 p-4 rounded-xl border border-slate-100 dark:border-slate-700">
                              <div className="flex items-baseline gap-2 mb-2">
                                  <span className="text-xs font-bold text-indigo-500 dark:text-indigo-400 bg-white dark:bg-slate-700 px-1.5 py-0.5 rounded border border-indigo-100 dark:border-indigo-800">{m.partOfSpeech}</span>
                                  <span className="font-bold text-slate-700 dark:text-slate-200">{m.definition}</span>
                              </div>
                              <p className="text-sm text-slate-500 dark:text-slate-400 italic font-serif">"{m.example}"</p>
                          </div>
                      ))}
                  </div>
              </div>
              ...
              <div className={`p-6 border-t ${spellingResult === 'correct' ? 'bg-emerald-50 dark:bg-emerald-900/20 border-emerald-100 dark:border-emerald-800' : spellingResult === 'incorrect' ? 'bg-rose-50 dark:bg-rose-900/20 border-rose-100 dark:border-rose-800' : 'bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800'}`}>
                  <form onSubmit={handleSpellingSubmit} className="relative">
                      <input 
                          ...
                          className={`w-full p-4 text-center text-xl font-bold rounded-xl border-2 outline-none transition-all ${
                              spellingResult === 'correct' ? 'border-emerald-500 text-emerald-600 dark:text-emerald-400 bg-white dark:bg-slate-800' :
                              spellingResult === 'incorrect' ? 'border-rose-500 text-rose-600 dark:text-rose-400 bg-white dark:bg-slate-800' :
                              'border-slate-200 dark:border-slate-700 focus:border-indigo-500 dark:focus:border-indigo-500 text-slate-800 dark:text-white bg-white dark:bg-slate-800'
                          }`}
                      />
                  ...
                  {spellingResult === 'incorrect' && (
                      <div className="mt-4 text-center animate-in fade-in slide-in-from-top-2">
                          ...
                          <p className="text-2xl font-black text-rose-600 dark:text-rose-400 mb-4">{word.term}</p>
                          <button 
        ...
        <div className="relative w-full h-full transition-transform duration-500 transform-style-3d origin-center" style={{ transform: isFlipped ? 'rotateY(180deg)' : 'rotateY(0deg)' }}>
            ...
            <div className="absolute inset-0 w-full h-full backface-hidden bg-white dark:bg-slate-900 rounded-3xl shadow-[0_15px_40px_-10px_rgba(0,0,0,0.08)] dark:shadow-none border border-slate-100 dark:border-slate-800 flex flex-col items-center justify-center p-8 overflow-hidden z-10 transition-colors">
                ...
                <div className="flex flex-col items-center text-center z-10 gap-5 w-full">
                    ...
                    {word.phonetic && (
                    <button onClick={(e) => { e.stopPropagation(); speakText(word.term); }} className="flex items-center gap-2 px-4 py-2 bg-slate-50 dark:bg-slate-800 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 rounded-full transition-all active:scale-95 group/audio border border-slate-100 dark:border-slate-800">
            ...
            <div className="absolute inset-0 w-full h-full backface-hidden bg-white dark:bg-slate-900 rounded-3xl shadow-[0_15px_40px_-10px_rgba(0,0,0,0.08)] dark:shadow-none border border-slate-100 dark:border-slate-800 flex flex-col overflow-hidden transition-colors" style={{ transform: 'rotateY(180deg)' }}>
                ...
                <div className="flex justify-between items-center px-6 py-4 pt-16 border-b border-slate-50 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm z-10">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); speakText(word.term); }} className="w-8 h-8 flex items-center justify-center bg-slate-50 dark:bg-slate-800 text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 rounded-full transition-colors"><Volume2 size={16} /></button>
                </div>
                ...
                <div className="flex-grow overflow-y-auto no-scrollbar p-6 space-y-5 bg-gradient-to-b from-white to-slate-50/30 dark:from-slate-900 dark:to-slate-900/50">
                    ...
                    <div key={idx} className="relative group/item">
                        ...
                        <div className="pl-3 border-l-2 border-slate-100 dark:border-slate-800 group-hover/item:border-indigo-200 dark:group-hover/item:border-indigo-800 transition-colors">
                            <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed italic font-serif">"{meaning.example}"</p>
                            <div className="flex items-center justify-between mt-1"><p className="text-xs text-slate-400 dark:text-slate-500 font-light">{meaning.translation}</p></div>
                        </div>
                    </div>
                    ))}
